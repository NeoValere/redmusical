...
import type { Prisma } from '@prisma/client'; // Correct import for Prisma types
...
export async function PUT(
  ...
  if (sessionError || !session) {
    console.error('API /api/m/[id]/update: Unauthorized - No active session or session error:', sessionError);
  ...
  try {
    ...
    if (!existingMusician) {
      console.error(`API /api/m/[id]/update: Musician with userId ${id} not found.`);
      return NextResponse.json({ message: 'Musician not found' }, { status: 404 });
    }

    if (existingMusician.userId !== userIdFromSession) {
      console.error(`API /api/m/[id]/update: Forbidden - User ${userIdFromSession} attempted to edit profile of ${existingMusician?.userId}`);
    ...
    if (!city) {
      return NextResponse.json({ message: 'Ciudad es requerida.' }, { status: 400 });
    }
    // Province is optional based on the schema (String?), so no mandatory validation for it here.
    ...
    // Check if the profile is autogenerated and the user is trying to claim it
    if (existingMusician.isAutogenerated) {
    const updateData: Prisma.MusicianUpdateInput = {
      fullName,
      city,
      province,
      instruments: { set: instruments }, // Use set for array updates
      genres: { set: genres }, // Use set for array updates
      bio,
      hourlyRate, // Use the converted number
      availability: { set: availability }, // Use set for array updates
      youtubeUrl,
      soundcloudUrl,
      instagramUrl,
    ...
    } else if (existingMusician.userId !== userIdFromSession) {
      console.error(`API /api/m/[id]/update: Forbidden - User ${userIdFromSession} attempted to edit profile of ${existingMusician?.userId}`);
      return NextResponse.json({ message: 'Forbidden: You can only edit your own profile.' }, { status: 403 });
    }
