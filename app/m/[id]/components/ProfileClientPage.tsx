...
import SelectProfileForMessageModal from './SelectProfileForMessageModal';
...
export default function ProfileClientPage({
  ...
  const [availableRoles, setAvailableRoles] = useState({ isMusician: false, isContractor: false });
  ...
  const handleSendMessageClick = async () => {
    if (!currentUser) {
      enqueueSnackbar('Debes iniciar sesión para enviar mensajes.', { variant: 'info' });
      return;
    }
    ...
    try {
      ...
      if (roles.isMusician && roles.isContractor) {
        setIsSelectProfileModalOpen(true);
      } else if (roles.isMusician) {
        handleCreateConversation('musician');
      } else if (roles.isContractor) {
        handleCreateConversation('contractor');
      } else {
        enqueueSnackbar('Necesitas un perfil de músico o de búsqueda para enviar mensajes.', { variant: 'warning' });
      }
    } catch (error) {
      console.error('Error fetching user profiles:', error);
      enqueueSnackbar('Error al verificar tus perfiles.', { variant: 'error' });
    }
  };

  const handleCreateConversation = async (senderRole: 'musician' | 'contractor') => {
    if (!musicianProfile || !currentUser) return;
    ...
    try {
      const response = await fetch('/api/conversations', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          recipientId: musicianProfile.userId,
          senderRole,
        }),
      });
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'No se pudo iniciar la conversación.');
      }
      ...
      enqueueSnackbar((error as Error).message || 'Error al iniciar la conversación.', { variant: 'error' });
    }
  };
      ...
      <Container maxWidth="lg" sx={{ py: isMobile ? 3 : 4 }}>
        <Box sx={{
          ...
          <Box sx={{ gridColumn: { xs: '1 / -1', md: 'span 4' } }}>
            ...
            >
              <Button style={{ marginBottom: "15px" }}  variant="contained" color="primary" onClick={handleSendMessageClick}>
                Mensajear
              </Button>
              {websiteUrl && (
            ...
            <SelectProfileForMessageModal
              open={isSelectProfileModalOpen}
              onClose={() => setIsSelectProfileModalOpen(false)}
              onSelectProfile={handleCreateConversation}
